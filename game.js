// Arquivo: game.js - VERSÃO COM SELEÇÃO DE PERSONAGEM REAL E FUNDO MELHORADO
...
let clouds = [];
let cityscape = [];
let mountains = []; // NOVO: Array para as montanhas
let particles = [];

// --- LÓGICA DE SELEÇÃO DE PERSONAGENS ---
// ATUALIZADO: Carregamos sua imagem para o primeiro personagem!
const playerImage = new Image();
playerImage.src = 'player_frame_0.png'; // Garante que este arquivo existe no seu GitHub

const characters = [
    { image: playerImage, name: 'Bolo' }, // Agora usa a imagem
    { color: '#ADD8E6', name: 'Azul' },    // Placeholder
    { color: '#90EE90', name: 'Verde' }     // Placeholder
];
...
function changeState(newState) {
    ...
    if (gameState === 'playing') {
        ...
        const size = char.image ? 48 : 40; // Personagens de imagem podem ser maiores
...
function update() {
    ...
    clouds.forEach(c => c.x -= pipeSpeed/3); if(frameCount % 200 === 0) generateCloud(); clouds = clouds.filter(c => c.x + 100 > 0);
    cityscape.forEach(b => b.x -= pipeSpeed/4); cityscape = cityscape.filter(b => b.x + b.width > 0);
    mountains.forEach(m => m.x -= pipeSpeed/6); mountains = mountains.filter(m => m.x + m.width > 0);
...
function drawCharSelect() {
    ...
    ctx.strokeText('Escolha seu Personagem', canvas.width / 2, 180);
    ctx.fillText('Escolha seu Personagem', canvas.width / 2, 180);

    // ATUALIZADO: Layout de seleção melhorado
    const spacing = 110;
    ...
    characters.forEach((char, index) => {
        ...
        // Desenha a imagem ou o placeholder
        ...
        if (index === selectedCharacterIndex) {
            ...
            ctx.lineWidth = 6;
            ctx.beginPath();
            ctx.arc(charX, charY, radius + 4, 0, Math.PI * 2);
            ctx.stroke();
        }
    });
...
function drawGame() {
    ...
    // ATUALIZADO: Desenha o personagem selecionado (imagem ou cor)
    ctx.save();
...
function drawBackground() {
    ...
    // NOVO: Desenha as montanhas
    ctx.fillStyle = '#1B2631';
    mountains.forEach(m => { ctx.beginPath(); ctx.moveTo(m.x, canvas.height); ctx.lineTo(m.x + m.width/2, m.y); ctx.lineTo(m.x + m.width, canvas.height); ctx.fill(); });

    ctx.fillStyle = '#1e3040'; cityscape.forEach(b => ctx.fillRect(b.x, b.y, b.width, b.height));
    
    // ATUALIZADO: Nuvens orgânicas de volta
...
// NOVO: Gerador de montanhas
function generateMountains() { let currentX = -100; while (currentX < canvas.width + 100) { const w = 150 + Math.random() * 100; const y = canvas.height - (120 + Math.random() * 80); mountains.push({x: currentX, y: y, width: w}); currentX += w - 50; } }
(function (){ generateMountains(); })(); // Gera as montanhas iniciais